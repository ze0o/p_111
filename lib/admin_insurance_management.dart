...
class _AdminInsuranceManagementPageState
  ...
  Widget _buildRequestsList(String status) {
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (context, snapshot) {
        print('Snapshot data: ${snapshot.data?.docs} ${status}');
        ...
        // return Center(
        //   child: Text(
        //     'No $status requests found $counter',
        //     style: TextStyle(fontSize: 18, color: AppTheme.textLight),
        //   ),
        // );

        if (snapshot.data != null) {
          return insurrance_cards(snapshot, status);
        }
                            ...
                            Container(
                              ...
                              child: Row(
                                children: [
                                  ...
                                  Expanded(
                                    child: Text(
                                      'Customer selected: ${data['selectedOfferType']} (\$${data['selectedOffer']})',
                                      style: TextStyle(
                                        fontWeight: FontWeight.bold,
                                        color: AppTheme.primaryColor,
                                      ),
  ...
  void _showCreateOffersDialog(
    ...
    final basicController = TextEditingController(
      text: (baseValue * 0.05 * accidentMultiplier).toString(),
    );
    final standardController = TextEditingController(
      text: (baseValue * 0.07 * accidentMultiplier).toString(),
    );
    final premiumController = TextEditingController(
      text: (baseValue * 0.10 * accidentMultiplier).toString(),
    );
  ...
  Future<void> _rejectRequest(String requestId) async {
        ...
        await showDialog(
          ...
          builder:
              (context) => AlertDialog(
                ...
                content: Text(
                  'Are you sure you want to reject this insurance request?',
                ),
                actions: [
                  TextButton(
    ...
    try {
      ...
      await FirebaseFirestore.instance
          ...
          .update({
            'status':
                'Rejected', // This is correct, but ensure UI handles this status
